...
| 1.3 | Replace hardcoded mock embeddingModel with config value            | P1       | done   | File: src/api/routes/query.ts - DEFAULT_QUERY_CONFIG has embeddingModel: "mock"                         |
| 1.4 | Update server to accept DaemonConfig and propagate to routes       | P1       | done   | File: src/api/server.ts - only accepts ApiConfig but routes need DaemonConfig                           |
