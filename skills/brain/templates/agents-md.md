...
- **Committer configuration**: `.memory/config.yaml` may define `committer.model`, `committer.thinking`, and `committer.extensions`. If omitted, Brain uses the active Pi session model and thinking, and forwards session extensions except this package. Set `committer.extensions` to replace that forwarded list. Pi handles authentication for the selected provider; do not add credentials to Brain configuration.
