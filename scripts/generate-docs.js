...
async function generateDocs() {
  ...
  if (!agentOnly) {
    ...
    try {
      await execCommand('npx', [
        'api-documenter',
        'markdown',
        '--input-folder', join(rootDir, 'docs/api'),
        '--output-folder', join(rootDir, 'docs/api/markdown'),
      ]);
    } catch (error) {
      console.warn('⚠️  Markdown generation failed (this is OK if no .api.json exists yet):', error.message);
    }
  }
  ...
  console.log('');
  console.log('For agents: Import the JSON doc model to explore the API programmatically.');
}
