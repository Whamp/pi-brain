...
src/config/config.ts [1-901]
  class:
    637-645: class ConfigError extends Error [exported]
  ...
  function:
    ...
    598-632: transformConfig(raw: RawConfig): PiBrainConfig [exported]
      /** Transform raw YAML config to typed config with validation */
    650-705: loadConfig(configPath?: string): PiBrainConfig [exported]
      /** Load configuration from a YAML file */
    710-715: ensureConfigDir(configDir?: string): void [exported]
      /** Ensure the config directory exists */
    720-755: ensureDirectories(config: PiBrainConfig): void [exported]
      /** Ensure all required directories exist based on configuration */
    760-819: writeDefaultConfig(configPath?: string): void [exported]
      /** Write a default configuration file */
    824-832: getSessionDirs(config: PiBrainConfig): {} [exported]
      /** Get all session directories to watch (hub + enabled spokes) */
    837-839: getEnabledSpokes(config: PiBrainConfig): {} [exported]
      /** Get enabled spokes from configuration */
    844-848: getRsyncSpokes(config: PiBrainConfig): {} [exported]
      /** Get rsync spokes (enabled spokes with rsync sync method) */
    853-860: getScheduledRsyncSpokes(config: PiBrainConfig): {} [exported]
      /** Get scheduled rsync spokes (rsync spokes with a schedule) */
    871-900: getComputerFromPath(sessionPath: string, config: PiBrainConfig): string [exported]
...
---
Files: 91
Estimated tokens: 27,289 (codebase: ~1,093,386)
